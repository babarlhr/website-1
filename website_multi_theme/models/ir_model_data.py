        ...
        if vals.get('model') == 'ir.ui.view' \
           and not self.env.context.get('duplicate_view_for_website'):
            ...
            if view.customize_show:
                ...
                # new module installation. It's also needed for CI tests when
                # another modules expects new item in "Customize menu".

                # We cannot override ir.ui.view model to do it, because it's
                # created before creating ir.model.data, which is essential for
                # multi_theme_reload

                # FIXME: themes are reloaded as much times as new module has
                # views with customize_show
